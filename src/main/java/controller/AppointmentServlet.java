...
public class AppointmentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            List<Lawyer> lawyers = lawyerDAO.getAvailableLawyers();
            if (lawyers == null || lawyers.isEmpty()) {
                System.out.println("No available lawyers found in database, will rely on JSP fallback");
            }
            request.setAttribute("lawyers", lawyers);

            // Log the request
            System.out.println("AppointmentServlet: Processing request for " + request.getRequestURI());
            ...
            System.err.println("AppointmentServlet Error: " + e.getMessage());
            System.err.println("Stack Trace: " + stackTrace);

            // Set error details in request
            request.setAttribute("errorMessage", "Unable to load appointment page. Please try again later.");
